...
setwd("~/Documents/GitHub/ps9-joehoang1")
...
names(ipip)
ipip.l <- ipip %>% gather(Trait, Score, A_1:O_10)
...
ipip.l <- ipip.l %>% separate(Trait, into=c('trait','Item'), sep="_")
...
ipip.comp <- ipip %>% select('RID', 'age', 'gender', 'BMI', 'exer', 'logMedInc') %>% left_join(ipip.comp)  
...
#I'm a little bit confused about the wording of this section
#I did it by the order that makes sense
#Here is the code if I was supposed to assign the levels by frequency 
table(ipip.comp$exer)
#Frequency descending: less1mo, more5wk, less1wk, veryRarelyNever, 1or2wk, 3or5wk
#ipip.comp$exer <- factor(ipip.comp$exer, levels=c('less1mo',"more5wk",less1wk','veryRarelyNever','1or2wk','3or5wk')) 
...
exer.avg <- ipip.comp %>% group_by(exer,trait) %>% 
                          summarise(
                                      n=length(RID),
                                      avg=mean(Avg),
                                      sem=sd(Avg)/sqrt(n-1))
...
gender.avg <- ipip.comp %>% group_by(gender,trait) %>% 
                            summarise(
                                      n=length(RID),
                                      avg=mean(Avg),
                                      sem=sd(Avg)/sqrt(n-1))
...
#Decided to use the mutate function instead!  
ipip.comp <- ipip.comp %>% mutate(BMI.1=cut(BMI, breaks=c(-Inf, 18, 25, 30, Inf), 
                                                     labels=c('underweight','healthy','overweight','obese')))
# turn BMI_cat into a factor and order it with levels
ipip.comp$BMI_cat <- factor(ipip.comp$BMI.1, levels=c('underweight','healthy','overweight','obese')) 

# summarise trait values by BMI categories  
bmi.avg <- ipip.comp %>% group_by(BMI_cat,trait) %>% 
                                                summarise(
                                                  n=length(RID),
                                                  avg=mean(Avg),
                                                  sem=sd(Avg)/sqrt(n-1))
...
computecor <- ipip.comp %>% spread(trait, Avg) %>%
                         select(age, A, C, E, N, O) %>% 
                         cor()
computecor

trait <- c("A","C","E","N","O")
corrcoef <- c(0.03472169,0.10897987,0.04111074,-0.1239703,-0.005111711)
age.avg <- data.frame(trait, corrcoef)
